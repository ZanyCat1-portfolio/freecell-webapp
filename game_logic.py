...
def can_place_on(dest_col, moving_stack):
    if not dest_col:
        if moving_stack[0].rank != 'K':
...
def move_cards(tableau, num_cards, from_col_idx, to_col_idx, freecells=None):
    ...
    if not valid_stack:
        return False, reason
    valid_place, reason = can_place_on(to_col, moving_stack)
    if not valid_place:
        return False, reason

    # Enforce max movable rule (must pass freecells as param)
    if freecells is not None:
        empty_freecells = sum(1 for c in freecells if c is None)
        # Only count empty tableau columns as helpers if bottom card is King
        bottom_card = moving_stack[0]
        # Don't count the source or destination columns
        empty_tableaus = sum(
            1 for i, col in enumerate(tableau)
            if len(col) == 0 and i != from_col_idx and i != to_col_idx
        )
        if len(to_col) == 0:
            # Only allow moving to empty column if led by a King
...
def move_from_freecell_to_tableau(freecells, tableau, freecell_idx, to_col_idx):
    card = freecells[freecell_idx]
    if card is None:
        return False, "Selected freecell is empty."
    valid_place, reason = can_place_on(tableau[to_col_idx], [card])
    if not valid_place:
        return False, reason
